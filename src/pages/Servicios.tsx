...
import { Plus, Search, Edit, Trash2, Stethoscope, X, AlertCircle, Clock, DollarSign, Eye, Filter, ChevronLeft, ChevronRight, Check } from 'lucide-react';
...
export default function Servicios({ user }: ServiciosProps) {
  const { isDark, bgCard, textPrimary, textSecondary, border, inputBg, inputBorder, inputText, bgSecondary, modalBg } = useDarkMode();
    ...
    if (storedServicios.length > 0) {
      ...
      const serviciosIniciales: Servicio[] = [
        { 
          ...
          descripcion: 'Consulta médica general con evaluación completa del estado de salud del paciente', 
          categoria: 'Consultas',
          categoriaColor: '#14B8A6',
          duracion: 30, 
          precio: 50000, 
        ...
        { 
          ...
          codigo: 'SRV002',
          nombre: 'Vacunación', 
          descripcion: 'Aplicación de vacunas según esquema de inmunización requerido', 
          categoria: 'Preventivos',
          categoriaColor: '#0EA5E9',
          duracion: 15, 
          precio: 30000, 
          imagen: 'https://images.unsplash.com/photo-1631549916768-4119b2e5f926?w=400&h=400&fit=crop',
          estado: 'Activo' 
        },
        { 
          id: '3', 
          ...
          descripcion: 'Medición de presión arterial y evaluación de signos vitales', 
          categoria: 'Diagnósticos',
          categoriaColor: '#8B5CF6',
          duracion: 10, 
          precio: 10000,
          imagen: 'https://images.unsplash.com/photo-1584467735871-8e85353a8413?w=400&h=400&fit=crop', 
          estado: 'Activo' 
        },
        { 
          id: '4', 
        ...
        { 
          ...
          descripcion: 'Aplicación de medicamentos vía intramuscular o intravenosa', 
        ...
        { 
          ...
          codigo: 'SRV006',
          nombre: 'Curaciones', 
          descripcion: 'Curación de heridas, quemaduras y procedimientos post-quirúrgicos', 
          categoria: 'Tratamientos',
          categoriaColor: '#EC4899',
          duracion: 20, 
          precio: 25000,
          imagen: 'https://images.unsplash.com/photo-1551601651-bc60f254d532?w=400&h=400&fit=crop', 
          estado: 'Activo' 
        },
      ];
  ...
  const categorias = ['Consultas', 'Preventivos', 'Diagnósticos', 'Tratamientos'];

  const validateCodigo = (codigo: string) => {
    if (!codigo) return 'Requerido';
    const exists = servicios.some(s => 
      s.codigo && s.codigo.toLowerCase() === codigo.toLowerCase() && 
  ...
  const isFormValid = () => {
    if (!formData.codigo || !formData.nombre || !formData.duracion || !formData.precio) return false;
  ...
  const openCreateModal = () => {
    ...
    setFormData({ codigo: nuevoCodigo, nombre: '', descripcion: '', categoria: '', duracion: '', precio: '', imagen: '' });
    setImagePreview(null);
    setErrors({ codigo: '', nombre: '', duracion: '', precio: '' });
    setTouched({ codigo: false, nombre: false, duracion: false, precio: false });
    setModalOpen(true);
  };
  ...
  const toggleEstadoServicio = async (servicio: Servicio) => {
    ...
    toast.success(`Servicio ${nuevoEstado === 'Activo' ? 'activado' : 'desactivado'} exitosamente`, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`lg:col-span-2 ${bgCard} rounded-xl p-4 border ${border} shadow-sm transition-colors duration-300`}>
        ...
        <div className={`${bgCard} rounded-xl p-4 border ${border} shadow-sm transition-colors duration-300`}>
          <div className="flex items-center gap-3">
            ...
            <Select value={estadoFiltro} onValueChange={(value) => {
              setEstadoFiltro(value);
              setCurrentPage(1);
            }}>
              <SelectTrigger className={`h-11 rounded-xl border-2 ${inputBorder} ${inputBg} ${inputText} focus:border-[#14B8A6]`}>
                <SelectValue placeholder="Estado" />
              </SelectTrigger>
              <SelectContent className={bgCard}>
                <SelectItem value="Todos" className={textPrimary}>Todos</SelectItem>
                <SelectItem value="Activo" className={textPrimary}>Activos</SelectItem>
                <SelectItem value="Inactivo" className={textPrimary}>Inactivos</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paginatedServicios.length === 0 ? (
          <div className={`col-span-full text-center p-8 ${textSecondary} ${bgCard} border ${border} rounded-xl`}>
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                {servicio.imagen ? (
                  ...
                  <div className={`w-full h-full flex items-center justify-center ${isDark ? 'bg-[#161b22]' : 'bg-gray-100'}`}>
                ...
                <div className="absolute top-3 right-3">
                  {servicio.estado === 'Activo' ? (
                    <span className="px-3 py-1 rounded-full bg-[#14B8A6] text-white shadow-lg" style={{ fontSize: '12px', fontWeight: 600 }}>
                      Disponible
                    </span>
                  ) : (
                    <span className="px-3 py-1 rounded-full bg-gray-500 text-white shadow-lg" style={{ fontSize: '12px', fontWeight: 600 }}>
                      No disponible
                    </span>
                  )}
                </div>
              ...
              <div className="p-5 space-y-4">
                <div>
                  <div className="flex items-start justify-between mb-2">
                    <h3 className={`${textPrimary} flex-1`} style={{ fontSize: '18px', fontWeight: 700 }}>
                      {servicio.nombre}
                    </h3>
                    <CategoryBadge 
                  ...
                  <p className={`${textSecondary} line-clamp-2`} style={{ fontSize: '13px' }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className={`p-3 rounded-xl ${isDark ? 'bg-[#161b22]' : 'bg-gray-50'}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <Clock className="w-4 h-4 text-[#14B8A6]" />
                      <span className={`${textSecondary}`} style={{ fontSize: '12px' }}>Duración</span>
                    </div>
                  ...
                  <div className={`p-3 rounded-xl bg-[#14B8A6] bg-opacity-10`}>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`${textSecondary}`} style={{ fontSize: '12px' }}>Precio</span>
                    </div>
                    <span className="text-[#14B8A6]" style={{ fontSize: '16px', fontWeight: 700 }}>
                ...
                <div className="grid grid-cols-3 gap-2 pt-2">
                  <Button
                    ...
                    className={`h-10 rounded-xl ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#3D4756]'}`}
      ...
      {/* Mejora 1: Paginación */}
      {totalPages > 1 && (
        <div className={`${bgCard} rounded-xl border ${border} p-4 shadow-sm transition-colors duration-300`}>
          <div className="flex items-center justify-between">
            <p className={textSecondary} style={{ fontSize: '14px' }}>
              Mostrando {((currentPage - 1) * itemsPerPage) + 1} - {Math.min(currentPage * itemsPerPage, filteredServicios.length)} de {filteredServicios.length} servicios
            </p>
      ...
      <Dialog open={detalleModalOpen} onOpenChange={setDetalleModalOpen}>
        <DialogContent className={`${modalBg} sm:max-w-[640px] rounded-xl border-2 ${border}`}>
          {selectedServicio && (
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`${textSecondary}`} style={{ fontSize: '13px' }}>Código: {selectedServicio.codigo}</p>
                  </div>
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="space-y-5 mt-6">
                ...
                <div className="relative h-64 rounded-xl overflow-hidden">
                  {selectedServicio.imagen ? (
                    ...
                    <div className={`w-full h-full flex flex-col items-center justify-center ${isDark ? 'bg-[#161b22]' : 'bg-gray-100'} border-2 border-dashed ${border}`}>
                      <Stethoscope className="w-20 h-20 text-gray-400 mb-3" />
                      <p className={`${textSecondary}`} style={{ fontSize: '13px' }}>Sin imagen</p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className={`${textPrimary} flex-1 min-w-0`} style={{ fontSize: '24px', fontWeight: 700, lineHeight: '1.2' }}>
                  ...
                  <p className={textSecondary} style={{ fontSize: '14px', lineHeight: '1.6' }}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className={`p-4 rounded-xl ${isDark ? 'bg-[#161b22]' : 'bg-gray-50'}`}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={textSecondary} style={{ fontSize: '13px', fontWeight: 500 }}>Precio</span>
                    </div>
                    <span className="text-[#14B8A6] block" style={{ fontSize: '28px', fontWeight: 700, lineHeight: '1' }}>
                  ...
                  <div className={`p-4 rounded-xl ${isDark ? 'bg-[#161b22]' : 'bg-gray-50'}`}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={textSecondary} style={{ fontSize: '13px', fontWeight: 500 }}>Duración</span>
                    </div>
                    <span className={`${textPrimary} block`} style={{ fontSize: '28px', fontWeight: 700, lineHeight: '1' }}>
                      {selectedServicio.duracion} <span style={{ fontSize: '16px', fontWeight: 500 }}>min</span>
                    </span>
                  </div>
                </div>
                ...
                <div className={`p-4 rounded-xl ${isDark ? 'bg-[#161b22]' : 'bg-gray-50'}`}>
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <span 
                        className={`${selectedServicio.estado === 'Activo' ? 'text-[#14B8A6]' : textSecondary}`}
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className={`${modalBg} sm:max-w-[600px] rounded-xl border-2 ${border} max-h-[90vh] overflow-y-auto`}>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${textSecondary} text-sm`}>Complete la información del servicio</p>
              </div>
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-[140px_1fr] gap-4">
                <div className={`w-full h-32 rounded-xl border-2 border-dashed ${border} flex items-center justify-center overflow-hidden ${isDark ? 'bg-[#0d1117]' : 'bg-gray-50'}`}>
                  {imagePreview ? (
                    ...
                    <div className="text-center p-2">
                      ...
                      <p className={`${textSecondary}`} style={{ fontSize: '10px' }}>Sin imagen</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col justify-center">
                  ...
                  />
                  <p className={`${textSecondary}`} style={{ fontSize: '11px' }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Código</label>
                <Input
                  ...
                  className={`h-11 rounded-xl border-2 ${inputBg} ${inputText} ${
                    touched.codigo && errors.codigo ? 'border-red-500' : inputBorder
                  }`}
                />
                {touched.codigo && errors.codigo && (
                  <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.codigo}</p>
                )}
              </div>

              <div>
                <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Categoría</label>
                <Select 
                ...
                >
                  ...
                  <SelectContent className={bgCard}>
                    {categorias.map(cat => (
                      <SelectItem key={cat} value={cat} className={textPrimary}>{cat}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div>
              <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Nombre del Servicio</label>
              <Input
                ...
                className={`h-11 rounded-xl border-2 ${inputBg} ${inputText} ${
                  touched.nombre && errors.nombre ? 'border-red-500' : inputBorder
                }`}
              />
              {touched.nombre && errors.nombre && (
                <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.nombre}</p>
              )}
            </div>

            <div>
              <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Descripción</label>
              <Textarea
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Duración (minutos)</label>
                <Input
                  type="number"
                  ...
                  placeholder="30"
                  className={`h-11 rounded-xl border-2 ${inputBg} ${inputText} ${
                    touched.duracion && errors.duracion ? 'border-red-500' : inputBorder
                  }`}
                />
                {touched.duracion && errors.duracion && (
                  <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.duracion}</p>
                )}
              </div>

              <div>
                <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Precio</label>
                <Input
                  type="number"
                  step="0.01"
                  ...
                  placeholder="50000"
                  className={`h-11 rounded-xl border-2 ${inputBg} ${inputText} ${
                    touched.precio && errors.precio ? 'border-red-500' : inputBorder
                  }`}
                />
                {touched.precio && errors.precio && (
                  <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.precio}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <Button
                ...
                className={`h-11 rounded-xl ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#374151]'}`}
              ...
              <Button
                ...
                style={{ fontWeight: 600 }}
              >
                {loading ? 'Guardando...' : selectedServicio ? 'Guardar Cambios' : 'Crear Servicio'}
              </Button>
            </div>
          </form>
        </DialogContent>
        ...
        <AlertDialogContent className={`${modalBg} rounded-xl border-2 ${border}`}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className={`${textSecondary} ml-16`} style={{ fontSize: '14px' }}>
              Se eliminará permanentemente "{selectedServicio?.nombre}". Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
