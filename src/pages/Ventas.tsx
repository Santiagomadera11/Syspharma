...
import { 
  Search, Plus, Trash2, DollarSign, ShoppingCart, Eye, Edit, Printer, 
  X, CheckCircle, Calendar, Filter, FileText, RotateCcw, AlertCircle,
  CreditCard, Package, ArrowLeft, Minus, TrendingUp, TrendingDown,
  DollarSign as DollarIcon, Lock, Download, User, Users
} from 'lucide-react';
...
import { TablaProductosVenta, ProductoVenta } from '../components/ventas/TablaProductosVenta';
import { TicketImprimible } from '../components/ventas/TicketImprimible';
import { motion, AnimatePresence } from 'motion/react';
import { ventasStorage, productosStorage, categoriasStorage } from '../utils/localStorage';
...
const clientesDB = [
  { cedula: '1020304050', nombre: 'Laura Fernández', telefono: '3001234567', direccion: 'Calle 45 #12-34' },
  { cedula: '1030405060', nombre: 'Roberto Díaz', telefono: '3109876543', direccion: 'Carrera 23 #56-78' },
  { cedula: '1040506070', nombre: 'María García', telefono: '3201234567', direccion: 'Avenida 68 #34-12' },
  { cedula: '1050607080', nombre: 'Carlos López', telefono: '3159876543', direccion: 'Calle 100 #45-67' },
  { cedula: '1060708090', nombre: 'Ana Martínez', telefono: '3181234567', direccion: 'Carrera 7 #23-45' },
  { cedula: '1070809000', nombre: 'Pedro Ramírez', telefono: '3229876543', direccion: 'Calle 72 #56-89' },
];
...
const generarVentasEjemplo = (): Venta[] => {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const descuentoTotal = productosVenta.reduce((sum, p) => 
    ...
    fechaVenta.setHours(Math.floor(Math.random() * 12) + 8, Math.floor(Math.random() * 60), 0, 0);
    ...
    ventas.push({
      ...
      total,
      metodoPago,
      montoPagado: metodoPago === 'Efectivo' ? Math.ceil(total / 1000) * 1000 : undefined,
      cambio: metodoPago === 'Efectivo' ? (Math.ceil(total / 1000) * 1000) - total : undefined,
...
export default function Ventas({ user }: VentasProps) {
  ...
  useEffect(() => {
    ...
    if (storedVentas.length > 0) {
      ...
      const ventasConFechas = storedVentas.map(v => ({
        ...
        fechaAnulacion: v.fechaAnulacion ? new Date(v.fechaAnulacion) : undefined,
  ...
  useEffect(() => {
    if (ventas.length > 0) {
      const ventasParaGuardar = ventas.map(v => ({
        ...
        estado: v.estado === 'Completada' ? 'Completada' as const : 'Cancelada' as const,
  ...
  const [gastoParaEliminar, setGastoParaEliminar] = useState<Gasto | null>(null);
  
  // Estados de búsqueda y filtros
  const [busqueda, setBusqueda] = useState('');
  const [cedulaCliente, setCedulaCliente] = useState('');
  const [clienteEncontrado, setClienteEncontrado] = useState<typeof clientesDB[0] | null>(null);
  ...
  const [productoSeleccionado, setProductoSeleccionado] = useState<typeof productosDisponibles[0] | null>(null);
  ...
  const ventasDelDia = ventas.filter(v => {
    ...
    return fechaVenta.toDateString() === hoy.toDateString() && v.estado === 'Completada';
  });
  ...
  const productosFiltrados = productosParaMostrar.filter(p => {
    const nombreProducto = p.nombreComercial || p.nombre || p.nombreGenerico || '';
    const coincideBusqueda = nombreProducto.toLowerCase().includes(busquedaProducto.toLowerCase()) ||
    ...
    return coincideBusqueda && (p.categoriaId === categoriaFiltro || p.categoria === categoriaFiltro);
  });
  ...
  const agregarProducto = (producto: typeof productosDisponibles[0]) => {
    ...
    if (productoExistente) {
      ...
      const nuevoProducto: ProductoVenta = {
        id: producto.id,
        nombre: producto.nombreComercial || producto.nombre || producto.nombreGenerico || 'Producto',
        precioUnitario: producto.precio,
        cantidad: 1,
  ...
  const actualizarCantidad = (id: string, cantidad: number) => {
    ...
    const nuevosProductos = formData.productos.map(p =>
      p.id === id
        ? { ...p, cantidad, subtotal: cantidad * p.precioUnitario * (1 - p.descuentoPorcentaje / 100) }
        : p
    );
    setFormData({ ...formData, productos: nuevosProductos });
  };
  ...
  const guardarVenta = async () => {
    if (!clienteEncontrado) {
      toast.error('Debes ingresar la cédula del cliente o usar cliente genérico', {
        style: { background: '#EF4444', color: 'white' }
      });
      return;
    }
    ...
    const nuevaVenta: Venta = {
      ...
      montoPagado: formData.metodoPago === 'Efectivo' ? formData.montoPagado : undefined,
  ...
  const ventasFiltradas = ventasDelDia.filter(venta => {
    ...
    return (
      ...
      (venta.fecha && new Date(venta.fecha).toLocaleDateString('es-ES').includes(busqueda))
    );
  });
  ...
  return (
    <div className={`min-h-screen ${bgPrimary} p-8`}>
      <div className="max-w-[1600px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className={textSecondary} style={{ fontSize: '14px', fontWeight: 600 }}>
              Ventas Brutas del Día
            </p>
            <p className="text-[#14B8A6] mt-2" style={{ fontSize: '36px', fontWeight: 700 }}>
              ${ventasBrutas.toLocaleString('es-CO')}
            </p>
            <p className={textSecondary} style={{ fontSize: '13px' }} className="mt-2">
          ...
          >
            ...
            <p className="text-orange-500 mt-2" style={{ fontSize: '36px', fontWeight: 700 }}>
          ...
          >
            ...
            <p className="text-green-500 mt-2" style={{ fontSize: '36px', fontWeight: 700 }}>
        ...
        {gastos.length > 0 && (
          <motion.div
            ...
            className={`${bgCard} rounded-3xl p-8 border-2 ${border} shadow-lg`}
          >
            <h3 className={`${textPrimary} mb-6`} style={{ fontSize: '20px', fontWeight: 700 }}>
              Gastos del Día
            </h3>
            ...
            <div className="space-y-3">
              {gastos.map((gasto, index) => (
                <motion.div
                  ...
                  className={`flex items-center justify-between p-4 rounded-2xl ${isDark ? 'bg-[#1a1d2e]' : 'bg-gray-50'} border ${border}`}
                >
                  <div className="flex items-center gap-4 flex-1">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                      gasto.categoria === 'Materiales' ? 'bg-blue-500' :
                      gasto.categoria === 'Servicios' ? 'bg-purple-500' :
                      gasto.categoria === 'Reparto' ? 'bg-green-500' :
                      'bg-gray-500'
                    }`}>
                    ...
                    <div className="flex-1">
                      <p className={textPrimary} style={{ fontSize: '15px', fontWeight: 600 }}>
                        {gasto.descripcion}
                      </p>
                      <p className={textSecondary} style={{ fontSize: '13px' }}>
                        {gasto.categoria} • {new Date(gasto.fecha).toLocaleTimeString('es-ES', { 
                          hour: '2-digit', 
                          minute: '2-digit' 
                        })}
                      </p>
                    ...
                    <p className="text-orange-500" style={{ fontSize: '18px', fontWeight: 700 }}>
                      ...
                      <div className="flex justify-center">
                        <span
                          className={`px-4 py-2 rounded-xl text-white ${
                            venta.estado === 'Completada' ? 'bg-green-500' : 'bg-red-500'
                          }`}
        ...
        >
          <div className="p-8 space-y-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 100px)' }}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="text-center pt-2 border-t border-gray-300 dark:border-gray-600">
                      <Button
                        ...
                        className={`${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-200 hover:bg-gray-300'} ${textPrimary} rounded-xl h-12 px-6`}
                    ...
                    {mostrarCrearCliente && (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <p className="text-orange-500" style={{ fontSize: '14px', fontWeight: 600 }}>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className={`${textPrimary} mb-2 block`} style={{ fontSize: '13px', fontWeight: 600 }}>
                              Nombre Completo *
                            </label>
                            <Input
                              type="text"
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        <p className="text-green-600 dark:text-green-400 mb-2" style={{ fontSize: '14px', fontWeight: 600 }}>
                          {usarClienteGenerico ? '👥 Cliente Genérico' : '✓ Cliente Encontrado'}
                        </p>
                        <p className={textPrimary} style={{ fontSize: '18px', fontWeight: 700 }}>
                          {clienteEncontrado.nombre}
                        </p>
                        <div className={`${textSecondary} mt-2 space-y-1`} style={{ fontSize: '13px' }}>
            ...
            <div className={`${bgCard} rounded-2xl p-6 border-2 ${border}`}>
              ...
              >
                <SelectTrigger className={`h-14 rounded-xl border-2 ${inputBorder} ${inputBg} ${inputText}`}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className={textPrimary} style={{ fontSize: '14px', fontWeight: 600 }}>
                              {producto.nombreComercial || producto.nombre || producto.nombreGenerico}
                            </p>
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700">
                      ...
                      <span className={textPrimary} style={{ fontSize: '16px', fontWeight: 700 }}>
                        ${calcularSubtotal(formData.productos).toLocaleString('es-CO')}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700">
                      ...
                      <span className="text-red-500" style={{ fontSize: '16px', fontWeight: 700 }}>
                        -${calcularDescuentoTotal(formData.productos).toLocaleString('es-CO')}
                      </span>
                    </div>
                    
                    <div className="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700">
                      <span className={textSecondary} style={{ fontSize: '14px' }}>IVA (16%):</span>
                      <span className={textPrimary} style={{ fontSize: '16px', fontWeight: 700 }}>
                        ${calcularIva(formData.productos).toLocaleString('es-CO')}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-3 border-t-2 border-[#14B8A6]">
                      ...
                      <span className="text-[#14B8A6]" style={{ fontSize: '28px', fontWeight: 700 }}>
                        ${calcularTotal(formData.productos).toLocaleString('es-CO')}
                      </span>
                    </div>
                  </div>
                  ...
                  {formData.metodoPago === 'Efectivo' ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className={`${textPrimary} mb-2 block`} style={{ fontSize: '13px', fontWeight: 700 }}>
                          Cliente pagó con:
                        </label>
                        <div className="relative">
                      ...
                      {formData.montoPagado >= calcularTotal(formData.productos) && formData.montoPagado > 0 && (
                        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-xl p-5 shadow-md">
                          <div className="text-white text-center">
                            <div style={{ fontSize: '13px', fontWeight: 600 }}>Cambio a devolver</div>
                            <div style={{ fontSize: '32px', fontWeight: 700 }} className="mt-1">
                      ...
                      {formData.montoPagado > 0 && formData.montoPagado < calcularTotal(formData.productos) && (
                        <div className="bg-red-500 bg-opacity-10 border-2 border-red-500 rounded-xl p-3 flex items-center gap-2">
                          <AlertCircle className="w-5 h-5 text-red-600 flex-shrink-0" />
                          <span className="text-red-600 dark:text-red-400" style={{ fontSize: '12px', fontWeight: 600 }}>
                            Monto insuficiente. Faltan ${(calcularTotal(formData.productos) - formData.montoPagado).toLocaleString('es-CO')}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-center">
                      <div className="text-center">
                        <CreditCard className={`w-16 h-16 mx-auto mb-3 ${textSecondary}`} />
            ...
            <div className="flex justify-end gap-3 pt-4 border-t-2 border-gray-200 dark:border-gray-700">
              ...
              <Button
                ...
                disabled={loading || formData.productos.length === 0 || !clienteEncontrado}
      ...
      <Dialog open={modalGastoOpen} onOpenChange={setModalGastoOpen}>
        <DialogContent className={`${bgCard} border-2 ${border} rounded-3xl`} style={{ maxWidth: '600px' }}>
          <DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: any) => setFormGasto({ ...formGasto, categoria: value })}
                >
      ...
      <Dialog open={modalCierreOpen} onOpenChange={setModalCierreOpen}>
        <DialogContent className={`${bgCard} border-2 ${border} rounded-3xl max-h-[90vh] overflow-y-auto`} style={{ maxWidth: '600px' }}>
          <DialogHeader>
            <DialogTitle className={`${textPrimary}`} style={{ fontSize: '24px', fontWeight: 700 }}>
              Cerrar Caja del Día
            </DialogTitle>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className={`${isDark ? 'bg-[#1a1d2e]' : 'bg-gray-50'} rounded-2xl p-6 space-y-3`}>
              <h4 className={`${textPrimary} mb-4`} style={{ fontSize: '16px', fontWeight: 700 }}>
                Resumen del Día
              </h4>
              
              <div className="flex justify-between items-center py-2 border-b border-gray-300 dark:border-gray-600">
                <span className={textSecondary} style={{ fontSize: '14px' }}>Ventas Brutas:</span>
                <span className="text-[#14B8A6]" style={{ fontSize: '18px', fontWeight: 700 }}>
                  ${ventasBrutas.toLocaleString('es-CO')}
                </span>
              </div>
              
              <div className="flex justify-between items-center py-2 border-b border-gray-300 dark:border-gray-600">
                <span className={textSecondary} style={{ fontSize: '14px' }}>Total Gastos:</span>
                <span className="text-orange-500" style={{ fontSize: '18px', fontWeight: 700 }}>
                  -${totalGastos.toLocaleString('es-CO')}
                </span>
              </div>
              
              <div className="flex justify-between items-center py-3 border-t-2 border-green-500">
                <span className={textPrimary} style={{ fontSize: '16px', fontWeight: 700 }}>Ganancia Neta:</span>
                <span className="text-green-500" style={{ fontSize: '24px', fontWeight: 700 }}>
                  ${gananciaNeta.toLocaleString('es-CO')}
                </span>
              </div>
              
              <div className="flex justify-between items-center py-2 mt-4 border-t border-gray-300 dark:border-gray-600">
                <span className={textSecondary} style={{ fontSize: '14px' }}>Efectivo Esperado:</span>
                <span className={textPrimary} style={{ fontSize: '16px', fontWeight: 700 }}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => setFormCierre({ ...formCierre, efectivoReal: Number(e.target.value) })}
                  className={`pl-11 h-14 rounded-xl border-2 ${inputBorder} ${inputBg} ${inputText} text-right`}
                  style={{ fontSize: '18px', fontWeight: 700 }}
                />
              </div>
              ...
              {formCierre.efectivoReal > 0 && (
                <div className={`mt-3 p-4 rounded-xl ${
                  ...
                  <div className="flex items-center justify-between">
                    <span className={textSecondary} style={{ fontSize: '14px' }}>Diferencia:</span>
                    <span className={`${
                      Math.abs(formCierre.efectivoReal - efectivoEsperado) < 1000
                        ? 'text-green-600'
                        : 'text-yellow-600'
                      ...
                      ${(formCierre.efectivoReal - efectivoEsperado).toLocaleString('es-CO')}
                    </span>
                  </div>
                </div>
              )}
            ...
            <>
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className={textSecondary} style={{ fontSize: '13px' }}>Fecha y Hora</p>
                    <p className={textPrimary} style={{ fontSize: '16px', fontWeight: 600 }}>
                      {new Date(selectedVenta.fecha).toLocaleDateString('es-ES')}
                    </p>
                    <p className={textSecondary} style={{ fontSize: '12px' }}>
                      {new Date(selectedVenta.fecha).toLocaleTimeString('es-ES')}
                    </p>
                  </div>
                </div>
                  ...
                  {selectedVenta.metodoPago === 'Efectivo' && (
                    <>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-green-500" style={{ fontWeight: 600 }}>
      ...
      <AlertDialog open={modalDevolucionOpen} onOpenChange={setModalDevolucionOpen}>
        <AlertDialogContent className={`${bgCard} border-2 ${border} rounded-3xl`}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className={`${textSecondary} text-center text-base mt-2`}>
              Esta acción generará una nota de crédito y anulará la venta. El proceso no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-6">
            <AlertDialogCancel className={`${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-200 hover:bg-gray-300'} ${textPrimary} rounded-xl h-12 px-6`}>
              Cancelar
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <AlertDialog open={modalEliminarVentaOpen} onOpenChange={setModalEliminarVentaOpen}>
        <AlertDialogContent className={`${bgCard} border-2 ${border} rounded-3xl`}>
          <AlertDialogHeader>
            <div className="w-16 h-16 rounded-2xl bg-red-100 dark:bg-red-900/20 flex items-center justify-center mx-auto mb-4">
            ...
            <AlertDialogDescription className={`${textSecondary} text-center text-base mt-2`}>
              Esta acción no se puede deshacer. La venta será eliminada de forma permanente del sistema.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-6">
            <AlertDialogCancel className={`${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-200 hover:bg-gray-300'} ${textPrimary} rounded-xl h-12 px-6`}>
      ...
      <AlertDialog open={modalEliminarGastoOpen} onOpenChange={setModalEliminarGastoOpen}>
        <AlertDialogContent className={`${bgCard} border-2 ${border} rounded-3xl`}>
          <AlertDialogHeader>
            <div className="w-16 h-16 rounded-2xl bg-red-100 dark:bg-red-900/20 flex items-center justify-center mx-auto mb-4">
              <Trash2 className="w-8 h-8 text-red-500" />
      ...
      {cajaCerrada && cierreCaja && (
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h4 style={{ fontSize: '16px', fontWeight: 700 }}>Caja Cerrada</h4>
