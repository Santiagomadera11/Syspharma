...
import { Plus, Search, Edit, Trash2, ArrowUpDown, ChevronLeft, ChevronRight, AlertTriangle, Building2, Phone, Mail, MapPin, FileText, Eye } from 'lucide-react';
...
import { proveedoresStorage, Proveedor as StorageProveedor } from '../utils/localStorage';
import { onlyNumbers, onlyLetters, validateNIT, validateEmail as validateEmailUtil, validatePhone } from '../utils/validation';
...
export default function Proveedores({ user }: ProveedoresProps) {
  ...
  useEffect(() => {
    ...
    const mapped = proveedoresFromStorage.map(p => ({
      ...
      logo: p.logo || `https://api.dicebear.com/7.x/initials/svg?seed=${p.nombre.substring(0,2)}&backgroundColor=14B8A6`
    }));
  ...
  const validateTelefono = (telefono: string) => {
    ...
    if (!phoneRegex.test(telefono.replace(/\s/g, ''))) return 'Formato inválido';
    return '';
  };
  ...
  const handleBlur = (field: keyof typeof touched) => {
    ...
    switch (field) {
      ...
      case 'nit': error = validateNit(formData.nit); break;
      case 'contacto': error = validateContacto(formData.contacto); break;
  ...
  const handleChange = (field: string, value: string) => {
    ...
    if (touched[field as keyof typeof touched]) {
      ...
      switch (field) {
        ...
        case 'nit': error = validateNit(value); break;
        case 'contacto': error = validateContacto(value); break;
  ...
  const isFormValid = () => {
    ...
    return !nombreError && !nitError && !contactoError && !telefonoError && !emailError;
  };
  ...
  const toggleEstado = (proveedor: Proveedor) => {
    ...
    toast.success(`Proveedor ${newEstado === 'Activo' ? 'activado' : 'desactivado'} exitosamente`, {
      style: { 
                ...
                {[
                  ...
                  >
                    <div className="flex items-center gap-2">
                      {label}
                      <ArrowUpDown className={`w-4 h-4 ${sortField === field ? 'text-white' : 'text-white/50'}`} />
                    </div>
                  </th>
                ))}
              ...
              {paginatedProveedores.length === 0 ? (
                ...
                paginatedProveedores.map((proveedor) => (
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <img 
                          ...
                          className={`w-10 h-10 rounded-full border-2 ${isDark ? 'border-[#161b22]' : 'border-white'}`}
                        />
                        <span 
                          className={`${textPrimary} ${proveedor.estado === 'Inactivo' ? 'line-through' : ''}`} 
                          style={{ fontWeight: 600, fontSize: '14px' }}
                        >
                          {proveedor.nombre}
                        </span>
                    ...
                    <td className={`p-4 ${textPrimary} ${proveedor.estado === 'Inactivo' ? 'line-through' : ''}`} style={{ fontSize: '14px' }}>
                      {proveedor.nit}
                    </td>
                    <td className={`p-4 ${textPrimary} ${proveedor.estado === 'Inactivo' ? 'line-through' : ''}`} style={{ fontSize: '14px' }}>
                      {proveedor.telefono}
                    ...
                    <td className="p-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span 
                          className={`text-sm ${proveedor.estado === 'Activo' ? 'text-[#14B8A6]' : textSecondary}`}
        ...
        {totalPages > 1 && (
          <div className={`flex items-center justify-between p-4 border-t ${border}`}>
            <p className={textSecondary} style={{ fontSize: '14px' }}>
              Mostrando {((currentPage - 1) * itemsPerPage) + 1} - {Math.min(currentPage * itemsPerPage, sortedProveedores.length)} de {sortedProveedores.length} proveedores
            </p>
          ...
          {selectedProveedor && (
            <div className="space-y-6 mt-4">
              ...
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <h3 className={`${textPrimary} text-xl font-bold`}>{selectedProveedor.nombre}</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className={`${textPrimary} font-semibold`}>{selectedProveedor.nit}</p>
                </div>
                <div>
                  <p className={`${textSecondary} text-xs mb-1`}>
                  ...
                  <p className={`${textPrimary} font-semibold`}>{selectedProveedor.contacto}</p>
                </div>
                <div>
                  <p className={`${textSecondary} text-xs mb-1`}>
                  ...
                  <p className={`${textPrimary} font-semibold`}>{selectedProveedor.telefono}</p>
                </div>
                <div>
                  <p className={`${textSecondary} text-xs mb-1`}>
                  ...
                  <p className={`${textPrimary} font-semibold`}>{selectedProveedor.email}</p>
                </div>
                {selectedProveedor.direccion && (
                  <div className="col-span-2">
                    ...
                    <p className={`${textPrimary} font-semibold`}>{selectedProveedor.direccion}</p>
                  </div>
                )}
              </div>
        ...
        <AlertDialogContent className={`${modalBg} rounded-xl border-2 ${border}`}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className={`${textSecondary} ml-16`} style={{ fontSize: '14px' }}>
              Se eliminará permanentemente a "{selectedProveedor?.nombre}". Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
