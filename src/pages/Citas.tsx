...
import { normalizarFecha, fechaAString, stringAFecha, mismaFecha } from '../utils/dateHelpers';
...
import { onlyNumbers, validateDNI, validateFullName, onlyLetters } from '../utils/validation';
...
type VistaPrincipal = 'calendario' | 'lista' | 'disponibilidad';
...
export default function Citas({ user }: CitasProps) {
  ...
  // Estados principales
  const [vistaPrincipal, setVistaPrincipal] = useState<VistaPrincipal>('calendario');
  ...
  const [fechaCalendarioDisponibilidad, setFechaCalendarioDisponibilidad] = useState(new Date());
  ...
  const citas = useMemo(() => {
    return citasGlobales.map(c => ({
      ...
      estado: c.estado as 'Pendiente' | 'Confirmada' | 'Cancelada' | 'Completada',
      notas: c.notas
    }));
  }, [citasGlobales]);
  ...
  const [filtroEmpleadoCalendario, setFiltroEmpleadoCalendario] = useState('Todos');
  ...
  // 🔴 IMPORTANTE: Obtener horas disponibles (ocultar las reservadas)
  ...
  const toggleDiaNoDisponible = (empleado: Empleado, fecha: Date) => {
    ...
    setEmpleados(empleados.map(e => 
      e.id === empleado.id 
        ? { ...e, diasNoDisponibles: nuevosDias }
        : e
    ));
  ...
  const validateCodigo = (codigo: string) => {
    ...
    const exists = citas.some(c => c.codigo === codigo && c.id !== selectedCita?.id);
  ...
  const handleBlur = (field: string) => {
    ...
    if (field === 'codigo') {
      ...
      setErrors({ ...errors, empleadoId: validateEmpleado(formData.empleadoId) });
  ...
  const handleChange = (field: string, value: any) => {
    ...
    if (field === 'servicio') {
      ...
      if (servicioData) {
        setFormData({ ...formData, servicio: value, duracion: servicioData.duracion });
      }
    }
  ...
  const isFormValid = () => {
    if (!formData.codigo || !formData.cliente || !formData.empleadoId || !formData.servicio || !formData.hora) return false;
    ...
    return !codigoError && !clienteError && !empleadoError && !servicioError && !horaError;
  };
  ...
  const openCreateModal = () => {
    ...
    setErrors({ codigo: '', cliente: '', empleadoId: '', servicio: '', hora: '' });
    setTouched({ codigo: false, cliente: false, empleadoId: false, servicio: false, hora: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTouched({ codigo: true, cliente: true, empleadoId: true, servicio: true, hora: true });
  ...
  const getHorariosDisponibles = () => {
    ...
    if (diaNoDisponible) {
      ...
      return HORARIOS_DIA.map(hora => ({ hora, disponible: false, ocupado: true }));
    }
    ...
    const citasDelDia = getCitasDelDia(formData.fecha).filter(c => 
  ...
  const citasFiltradas = citas.filter(c => {
    ...
    const matchesEmpleado = filtroEmpleado === 'Todos' || c.empleado === filtroEmpleado;
    const matchesServicio = filtroServicio === 'Todos' || c.servicio === filtroServicio;
  ...
  const getEstadoColor = (estado: Cita['estado']) => {
    switch (estado) {
      ...
      case 'Confirmada': return 'bg-[#14B8A6]';
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pestañas de vista */}
      <div className={`${bgCard} rounded-xl p-2 border ${border} shadow-sm transition-colors duration-300 inline-flex gap-2`}>
        <Button
          onClick={() => setVistaPrincipal('calendario')}
          className={`h-10 rounded-lg transition-all duration-200 ${
            vistaPrincipal === 'calendario'
              ? 'bg-[#14B8A6] text-white shadow-md'
              : `${isDark ? 'bg-transparent hover:bg-[#1f6feb1a] text-white' : 'bg-transparent hover:bg-gray-100 text-[#3D4756]'}`
          }`}
      ...
      {vistaPrincipal === 'calendario' && (
        <div className="space-y-4">
          ...
          <div className={`${bgCard} rounded-xl p-4 border ${border} shadow-sm transition-colors duration-300`}>
            {/* Filtro por médico */}
            <div className="mb-4">
              <label className={`block ${textSecondary} mb-2`} style={{ fontSize: '12px', fontWeight: 600 }}>
                Filtrar por médico:
              </label>
              <Select value={filtroEmpleadoCalendario} onValueChange={setFiltroEmpleadoCalendario}>
                <SelectTrigger className={`h-10 rounded-xl border-2 ${inputBorder} ${inputBg} ${inputText}`}>
                  <SelectValue placeholder="Todos los médicos" />
                </SelectTrigger>
                <SelectContent className={bgCard}>
                  <SelectItem value="Todos" className={textPrimary}>Todos los médicos</SelectItem>
                  {empleados.map(emp => (
                    <SelectItem key={emp.id} value={emp.id} className={textPrimary}>
                      {emp.nombre}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <Button
                  ...
                  className={`h-10 w-10 p-0 rounded-lg ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#3D4756]'}`}
                >
                  <ChevronLeft className="w-5 h-5" />
                </Button>
                <h3 className={textPrimary} style={{ fontSize: '18px', fontWeight: 700, minWidth: '180px', textAlign: 'center' }}>
              ...
              <div className="flex items-center gap-2">
                <Button
                  onClick={irAHoy}
                  className={`h-10 px-4 rounded-lg ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#3D4756]'}`}
                  style={{ fontWeight: 600, fontSize: '13px' }}
                >
                  Hoy
                </Button>

                <div className="flex gap-1 p-1 rounded-lg" style={{ background: isDark ? '#161b22' : '#f3f4f6' }}>
                  <Button
              ...
              <div className="grid grid-cols-7 gap-2 mb-4">
                {DIAS_SEMANA.map(dia => (
                  <div key={dia} className="text-center py-2">
                    <span className={textSecondary} style={{ fontSize: '13px', fontWeight: 600 }}>
                      {dia}
                    </span>
                  </div>
                ))}
                    ...
                    <motion.div
                      ...
                      className={`aspect-square rounded-xl p-2 cursor-pointer transition-all duration-200 ${
                        esSeleccionado
                          ...
                          : numCitas > 0
                          ? `${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a]' : 'bg-gray-50 hover:bg-gray-100'}`
                          : `${isDark ? 'hover:bg-[#161b22]' : 'hover:bg-gray-50'}`
                      }`}
                    >
                      <div className="flex flex-col h-full">
                        <span 
                          className={`${esSeleccionado ? 'text-white' : esHoy ? 'text-[#14B8A6]' : textPrimary}`}
                          style={{ fontSize: '14px', fontWeight: esHoy || esSeleccionado ? 700 : 500 }}
                        >
                          {dia.getDate()}
                        </span>
                        ...
                        {getCitasDelDia(dia).slice(0, 2).map(cita => (
                          <div 
                            ...
                            title={`${cita.hora} - ${cita.cliente} - ${cita.servicioNombre || cita.servicio}`}
                            onClick={(e) => {
                              e.stopPropagation();
                              setCitaDetalle(cita);
            ...
            <div className={`${bgCard} rounded-xl p-6 border ${border} shadow-sm transition-colors duration-300`}>
              <div className="mb-4">
                <h3 className={textPrimary} style={{ fontSize: '18px', fontWeight: 700 }}>
                  {getDiaNombre(fechaSeleccionada)}, {fechaSeleccionada.getDate()} de {MESES[fechaSeleccionada.getMonth()]}
                </h3>
              </div>

              <div className="space-y-2">
                {HORARIOS_DIA.map(hora => {
                  const citasHora = getCitasDelDia(fechaSeleccionada).filter(c => c.hora === hora);
                  
                  return (
                    <div key={hora} className="grid grid-cols-[80px_1fr] gap-4 items-start">
                      <div className={`${textSecondary} text-right pt-2`} style={{ fontSize: '13px', fontWeight: 600 }}>
                        {hora}
                      </div>
                      <div className={`min-h-[60px] rounded-xl border-2 border-dashed ${border} p-3 ${citasHora.length > 0 ? bgCard : ''}`}>
                        {citasHora.map(cita => {
                          ...
                          return (
                            <div
                              ...
                              className={`p-3 rounded-lg mb-2 last:mb-0 cursor-pointer transition-all hover:shadow-md ${getEstadoColor(cita.estado)} bg-opacity-10 border-l-4`}
                              style={{ borderLeftColor: getEstadoColor(cita.estado).replace('bg-', '#') }}
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-1">
                                    <Icon className="w-4 h-4" />
                                    <span className={textPrimary} style={{ fontSize: '14px', fontWeight: 700 }}>
                                      {cita.cliente}
                                    </span>
                                  </div>
                                  <p className={textSecondary} style={{ fontSize: '12px' }}>
                                    {cita.servicioNombre || cita.servicio} · {cita.duracion} min
                                  </p>
                                  <p className={textSecondary} style={{ fontSize: '12px' }}>
                                    {cita.empleado}
                                  </p>
                                ...
                                <span 
                                  className={`px-2 py-1 rounded-full ${getEstadoColor(cita.estado)} text-white`}
      ...
      {vistaPrincipal === 'lista' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className={`${bgCard} rounded-xl p-4 border ${border} shadow-sm`}>
            ...
            <Select value={filtroEstado} onValueChange={setFiltroEstado}>
              <SelectTrigger className={`h-11 rounded-xl border-2 ${inputBorder} ${inputBg} ${inputText}`}>
                <SelectValue placeholder="Estado" />
              </SelectTrigger>
              <SelectContent className={bgCard}>
                <SelectItem value="Todos" className={textPrimary}>Todos los estados</SelectItem>
                <SelectItem value="Pendiente" className={textPrimary}>Pendiente</SelectItem>
                <SelectItem value="Confirmada" className={textPrimary}>Confirmada</SelectItem>
                <SelectItem value="Completada" className={textPrimary}>Completada</SelectItem>
                <SelectItem value="Cancelada" className={textPrimary}>Cancelada</SelectItem>
              </SelectContent>
            </Select>
            ...
            <Select value={filtroEmpleado} onValueChange={setFiltroEmpleado}>
              ...
              <SelectContent className={bgCard}>
                <SelectItem value="Todos" className={textPrimary}>Todos los empleados</SelectItem>
                {empleados.map(emp => (
                  <SelectItem key={emp.id} value={emp.nombre} className={textPrimary}>
                    {emp.nombre}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <Select value={filtroServicio} onValueChange={setFiltroServicio}>
              ...
              <SelectContent className={bgCard}>
                <SelectItem value="Todos" className={textPrimary}>Todos los servicios</SelectItem>
                {servicios.map(srv => (
                  <SelectItem key={srv.nombre} value={srv.nombre} className={textPrimary}>
                    {srv.nombre}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {/* Tabla de citas */}
          <div className={`${bgCard} rounded-xl border ${border} shadow-sm overflow-hidden`}>
              ...
              <table className="w-full">
                <thead className="bg-gradient-to-r from-[#14B8A6] to-[#0EA5E9]">
                  <tr>
                    ...
                    <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>Fecha/Hora</th>
                    <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>Cliente</th>
                    <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>Empleado</th>
                    <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>Servicio</th>
                ...
                <tbody>
                  {citasFiltradas.length === 0 ? (
                    <tr>
                      <td colSpan={7} className={`text-center p-8 ${textSecondary}`}>
                    ...
                    citasFiltradas.map((cita) => {
                      ...
                      return (
                        ...
                        >
                          <td className={`p-4 ${textPrimary}`} style={{ fontSize: '13px', fontWeight: 600 }}>
                            {cita.codigo}
                          </td>
                          <td className={`p-4 ${textPrimary}`} style={{ fontSize: '13px' }}>
                            <div>{cita.fecha.toLocaleDateString('es-ES')}</div>
                            <div className={textSecondary} style={{ fontSize: '12px' }}>{cita.hora}</div>
                          </td>
                          <td className={`p-4 ${textPrimary}`} style={{ fontSize: '13px', fontWeight: 600 }}>
                            {cita.cliente}
                          </td>
                          <td className={`p-4 ${textSecondary}`} style={{ fontSize: '13px' }}>
                            {cita.empleado}
                          </td>
                          <td className={`p-4 ${textSecondary}`} style={{ fontSize: '13px' }}>
                          ...
                          <td className="p-4">
                            <span 
                              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full ${getEstadoColor(cita.estado)} text-white`}
                              style={{ fontSize: '12px', fontWeight: 600 }}
                            >
      ...
      {vistaPrincipal === 'disponibilidad' && (
        <div className="space-y-4">
          ...
          <div className={`${bgCard} rounded-xl p-4 border ${border} shadow-sm`}>
            <label className={`block ${textPrimary} mb-3`} style={{ fontSize: '14px', fontWeight: 600 }}>
                ...
                >
                  <div className="flex items-center gap-3">
                    {empleado.foto ? (
                      ...
                      <div className={`w-16 h-16 rounded-full ${isDark ? 'bg-[#161b22]' : 'bg-gray-200'} flex items-center justify-center`}>
                        <Stethoscope className="w-8 h-8 text-gray-400" />
                      </div>
                    )}
                    <div className="flex-1">
                      <h4 className={textPrimary} style={{ fontSize: '15px', fontWeight: 700 }}>
                        {empleado.nombre}
                      </h4>
                      <p className={textSecondary} style={{ fontSize: '12px' }}>
          ...
          {empleadoSeleccionado && (
            <>
              ...
              <CalendarioDisponibilidad
                ...
                onToggleDia={(fecha) => toggleDiaNoDisponible(empleadoSeleccionado, fecha)}
                isDark={isDark}
              ...
              <div className={`${bgCard} rounded-xl p-6 border ${border} shadow-sm`}>
                ...
                <div className="space-y-4">
                  {['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'].map(dia => {
                    const horariosDisponibles = empleadoSeleccionado.disponibilidad[dia] || [];
                    
                    return (
                      <div key={dia} className={`p-4 rounded-xl ${isDark ? 'bg-[#161b22]' : 'bg-gray-50'}`}>
                        ...
                        <div className="grid grid-cols-6 md:grid-cols-10 gap-2">
                          {HORARIOS_DIA.slice(0, 14).map(hora => {
                            const estaDisponible = horariosDisponibles.includes(hora);
                              ...
                              <button
                                ...
                                className={`h-10 rounded-lg transition-all duration-200 ${
                                  estaDisponible
                                    ...
                                    : `${isDark ? 'bg-[#0d1117] hover:bg-[#14B8A6]' : 'bg-white hover:bg-[#14B8A6]'} ${textSecondary} hover:text-white border ${border}`
                                }`}
                                style={{ fontSize: '11px', fontWeight: 600 }}
                              >
                                {hora}
      ...
      <Dialog open={citasDiaModalOpen} onOpenChange={setCitasDiaModalOpen}>
        <DialogContent className={`${modalBg} sm:max-w-[600px] rounded-xl border-2 ${border}`}>
          <DialogHeader>
          ...
          <div className="space-y-3 mt-4">
            {getCitasDelDia(fechaSeleccionada).length === 0 ? (
              <p className={`${textSecondary} text-center py-8`}>No hay citas programadas para este día</p>
            ) : (
              getCitasDelDia(fechaSeleccionada).map(cita => {
                ...
                return (
                  <div
                    key={cita.id}
                    className={`p-4 rounded-xl border ${border} ${isDark ? 'bg-[#161b22]' : 'bg-gray-50'} cursor-pointer hover:shadow-md transition-all`}
                    onClick={() => {
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={textPrimary} style={{ fontSize: '15px', fontWeight: 700 }}>
                            {cita.hora}
                          </span>
                      ...
                      <span 
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full ${getEstadoColor(cita.estado)} text-white shrink-0`}
                        style={{ fontSize: '12px', fontWeight: 600 }}
                      >
                        <Icon className="w-3.5 h-3.5" />
                        {cita.estado}
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className={`${modalBg} sm:max-w-[640px] rounded-xl border-2 ${border} max-h-[90vh] overflow-y-auto`}>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#14B8A6] to-[#0EA5E9] flex items-center justify-center shrink-0">
                <Calendar className="w-6 h-6 text-white" />
              ...
              <div className="flex-1">
                ...
                <p className={textSecondary} style={{ fontSize: '13px' }}>Complete la información de la cita</p>
              </div>
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={tipoDocumento} onValueChange={setTipoDocumento}>
                  <SelectTrigger className={`h-11 rounded-xl border-2 ${inputBorder} ${inputBg}`}>
                    <SelectValue placeholder="Selecciona tipo" />
                  </SelectTrigger>
                  <SelectContent position="popper" className={`${bgCard} z-[9999]`} sideOffset={5}>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNumeroDocumento(onlyNumbers(e.target.value))}
                  placeholder="Ej: 12345678"
                  className={`h-11 rounded-xl border-2 ${inputBorder} ${inputBg} ${inputText}`}
                  maxLength={10}
                />
                {numeroDocumento && validateDNI(numeroDocumento).message && (
                  <p className="text-red-500 text-xs mt-1">{validateDNI(numeroDocumento).message}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleChange('cliente', onlyLetters(e.target.value))}
                onBlur={() => handleBlur('cliente')}
                placeholder="Nombre completo"
                className={`h-11 rounded-xl border-2 ${inputBg} ${inputText} ${
                  touched.cliente && errors.cliente ? 'border-red-500' : inputBorder
                }`}
              />
              {touched.cliente && errors.cliente && (
                <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.cliente}</p>
              )}
            </div>

            {/* Selector de Empleado */}
            <div>
              <label className={`block ${textPrimary} mb-3`} style={{ fontSize: '13px', fontWeight: 600 }}>
                Empleado / Médico
              </label>
                ...
                {empleados.map(empleado => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (formData.fecha && esDiaNoDisponible(empleado, formData.fecha)) {
                        toast.error(`⚠️ ${empleado.nombre} NO está disponible en la fecha seleccionada (${fechaAString(formData.fecha)})`, {
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {empleado.foto ? (
                        ...
                        <div className={`w-12 h-12 rounded-full ${isDark ? 'bg-[#161b22]' : 'bg-gray-200'} flex items-center justify-center`}>
                          <Stethoscope className="w-6 h-6 text-gray-400" />
                        </div>
                      )}
                      <div className="flex-1">
                        <h4 className={textPrimary} style={{ fontSize: '14px', fontWeight: 700 }}>
                          {empleado.nombre}
                        </h4>
                        <p className={textSecondary} style={{ fontSize: '12px' }}>
                          {empleado.especialidad}
              ...
              {touched.empleadoId && errors.empleadoId && (
                <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.empleadoId}</p>
              )}
            </div>
            ...
            <div>
              <label className={`block ${textPrimary} mb-3`} style={{ fontSize: '13px', fontWeight: 600 }}>
                Servicio *
                {servicios.length === 0 && (
                  <span className="text-red-500 text-xs ml-2">(No hay servicios activos)</span>
                )}
              </label>
                ...
                {servicios.length === 0 ? (
                  ...
                  servicios.map(srv => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setFormData({ ...formData, servicio: srv.id, duracion: srv.duracion });
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div style={{ fontSize: '14px', fontWeight: 600 }}>{srv.nombre}</div>
                          <div className={`text-xs mt-1 ${formData.servicio === srv.id ? 'text-white/90' : 'text-gray-500'}`}>
              ...
              {touched.servicio && errors.servicio && (
                <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.servicio}</p>
              )}
            </div>
            ...
            <div>
              ...
              <CalendarioSeleccionFecha
                ...
                empleadoSeleccionado={empleados.find(e => e.id === formData.empleadoId) || null}
                onSelectFecha={(fecha) => {
                  ...
                  if (formData.empleadoId) {
                    ...
                    if (empleado && esDiaNoDisponible(empleado, fecha)) {
                      toast.error('⚠️ Este médico NO está disponible en la fecha seleccionada', {
                        style: { background: '#EF4444', color: 'white' },
                        duration: 4000
                      });
                    }
              ...
              {touched.fecha && errors.fecha && (
                <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.fecha}</p>
              )}
            </div>
            ...
            <div>
              <label className={`block ${textPrimary} mb-3`} style={{ fontSize: '13px', fontWeight: 600 }}>
                ...
                {horasDisponiblesParaFecha.length === 0 && (
                  <span className="text-red-500 text-xs ml-2">(No hay horas disponibles)</span>
                )}
              </label>
              ...
              {touched.hora && errors.hora && (
                <p className="text-red-500 mt-1.5" style={{ fontSize: '12px' }}>{errors.hora}</p>
              )}
            </div>

            {selectedCita && (
              <div>
                <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Estado</label>
                  ...
                  {(['Pendiente', 'Confirmada', 'Completada', 'Cancelada'] as const).map(estado => {
                    ...
                    return (
                      <button
                        key={estado}
                        type="button"
                        onClick={() => cambiarEstadoCita(selectedCita.id, estado)}
                        className={`h-11 rounded-xl flex items-center justify-center gap-2 transition-all ${
                          selectedCita.estado === estado
                            ...
                            : `${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-100 hover:bg-gray-200 text-[#3D4756]'} border ${border}`
                        }`}
                        style={{ fontSize: '13px', fontWeight: 600 }}
                      >
                        <Icon className="w-4 h-4" />
            ...
            <div>
              <label className={`block ${textPrimary} mb-2`} style={{ fontSize: '13px', fontWeight: 600 }}>Notas</label>
              <Textarea
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <Button
                ...
                className={`h-12 rounded-xl ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#374151]'} transition-all`}
              ...
              <Button
                ...
                style={{ fontWeight: 600, fontSize: '14px' }}
              >
                {loading ? 'Guardando...' : selectedCita ? 'Guardar Cambios' : 'Crear Cita'}
              </Button>
            </div>
          </form>
        </DialogContent>
      ...
      <Dialog open={detailModalOpen} onOpenChange={setDetailModalOpen}>
        <DialogContent className={`${modalBg} rounded-2xl p-6 max-w-md border-2 ${border}`}>
          <DialogHeader>
          ...
          {citaDetalle && (
            <div className="space-y-4 mt-4">
              <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                <label className={`block ${textSecondary} mb-1 text-xs uppercase tracking-wide`}>Código</label>
                <p className={`${textPrimary} font-bold text-lg`}>{citaDetalle.codigo}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                  <label className={`block ${textSecondary} mb-1 text-xs uppercase tracking-wide`}>Fecha</label>
                ...
                <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                  <label className={`block ${textSecondary} mb-1 text-xs uppercase tracking-wide`}>Hora</label>
                  <p className={`${textPrimary} font-bold text-lg`}>{citaDetalle.hora}</p>
                </div>
              </div>

              <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                <label className={`block ${textSecondary} mb-1 text-xs uppercase tracking-wide`}>Cliente</label>
                <p className={`${textPrimary} font-semibold text-lg`}>{citaDetalle.cliente}</p>
              </div>

              <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                <label className={`block ${textSecondary} mb-1 text-xs uppercase tracking-wide`}>Empleado/Médico</label>
                <p className={`${textPrimary} font-semibold`}>{citaDetalle.empleado}</p>
              </div>

              <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                <label className={`block ${textSecondary} mb-1 text-xs uppercase tracking-wide`}>Servicio</label>
                <p className={`${textPrimary} font-semibold`}>{citaDetalle.servicioNombre || citaDetalle.servicio}</p>
                <p className={`${textSecondary} text-sm mt-1`}>Duración: {citaDetalle.duracion} minutos</p>
              </div>

              <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                <label className={`block ${textSecondary} mb-2 text-xs uppercase tracking-wide`}>Estado</label>
              ...
              {citaDetalle.notas && (
                <div className={`${bgCard} rounded-xl p-4 border ${border}`}>
                  <label className={`block ${textSecondary} mb-2 text-xs uppercase tracking-wide`}>Notas</label>
                  <p className={`${textPrimary} text-sm leading-relaxed`}>{citaDetalle.notas}</p>
                </div>
              )}
        ...
        <AlertDialogContent className={`${modalBg} rounded-xl border-2 ${border}`}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className={`${textSecondary} ml-16`} style={{ fontSize: '14px' }}>
              La cita de "{selectedCita?.cliente}" se eliminará permanentemente. Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
