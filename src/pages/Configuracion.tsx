...
import { Save, Building2, User, Bell, Shield, Database, Edit, Trash2, X, AlertCircle, Calendar, Users as UsersIcon, Search, Filter, Plus, Info } from 'lucide-react';
...
const permisosDisponibles = {
  usuarios: [
    { id: 'ver_usuarios', nombre: 'Ver Usuarios', descripcion: 'Visualizar lista de usuarios' },
    { id: 'crear_usuarios', nombre: 'Crear Usuarios', descripcion: 'Agregar nuevos usuarios' },
    { id: 'editar_usuarios', nombre: 'Editar Usuarios', descripcion: 'Modificar usuarios existentes' },
    { id: 'eliminar_usuarios', nombre: 'Eliminar Usuarios', descripcion: 'Eliminar usuarios del sistema' },
  ],
  facturacion: [
    { id: 'ver_facturas', nombre: 'Ver Facturas', descripcion: 'Visualizar facturas' },
    { id: 'crear_facturas', nombre: 'Crear Facturas', descripcion: 'Generar nuevas facturas' },
    { id: 'anular_facturas', nombre: 'Anular Facturas', descripcion: 'Anular facturas existentes' },
  ],
  reportes: [
    { id: 'ver_reportes', nombre: 'Ver Reportes', descripcion: 'Acceso a reportes' },
    { id: 'exportar_reportes', nombre: 'Exportar Reportes', descripcion: 'Descargar reportes' },
  ],
  configuracion: [
    { id: 'ver_config', nombre: 'Ver Configuración', descripcion: 'Ver ajustes del sistema' },
    { id: 'editar_config', nombre: 'Editar Configuración', descripcion: 'Modificar configuración' },
    { id: 'gestionar_roles', nombre: 'Gestionar Roles', descripcion: 'Administrar roles y permisos' },
  ],
  inventario: [
    { id: 'ver_productos', nombre: 'Ver Productos', descripcion: 'Visualizar inventario' },
    { id: 'crear_productos', nombre: 'Crear Productos', descripcion: 'Agregar productos' },
    { id: 'editar_productos', nombre: 'Editar Productos', descripcion: 'Modificar productos' },
  ],
};
...
export default function Configuracion({ user }: ConfiguracionProps) {
  ...
  const [roles, setRoles] = useState<Rol[]>([
    {
      ...
      permisos: Object.values(permisosDisponibles).flat().map(p => p.id),
      usuariosActivos: 5,
      estado: true,
    ...
    },
    {
      ...
      permisos: ['ver_usuarios', 'ver_facturas', 'crear_facturas', 'ver_productos', 'crear_productos'],
      usuariosActivos: 12,
      estado: true,
  ...
  const handleBlur = (field: string) => {
    ...
    if (field === 'nombre') {
      ...
      setErrors({ ...errors, descripcion: validateDescripcion(formData.descripcion) });
    }
  };
  ...
  const openCreateModal = () => {
    setSelectedRol(null);
    // No resetear formData - mantener los datos ingresados en la tarjeta
    // Solo resetear los permisos si es necesario
    setFormData({
      ...formData,
      permisos: []
    });
  ...
  const toggleEstado = (rol: Rol) => {
    ...
    setRoles(roles.map(r => 
      r.id === rol.id ? { ...r, estado: !r.estado } : r
    ));
  ...
  return (
    <Tooltip.Provider>
      <div className="space-y-6">
        ...
        {/* Tarjeta de Crear Rol */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, delay: 0.1 }}
          className={`${bgCard} rounded-2xl shadow-lg border ${border} p-6 transition-colors duration-300`}
        >
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#63E6BE] to-[#3D4756] flex items-center justify-center shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <div 
                  className={`absolute right-4 top-1/2 transform -translate-y-1/2 w-6 h-6 rounded-full border-2 ${isDark ? 'border-[#161b22]' : 'border-white'} shadow-md`}
          ...
          <div className="flex gap-3">
            <Button
              onClick={() => {
                setFormData({ nombre: '', color: '#63E6BE', descripcion: '', permisos: [] });
                setErrors({ nombre: '', descripcion: '' });
                setTouched({ nombre: false, descripcion: false });
              }}
              className={`flex-1 h-11 rounded-xl transition-all duration-300 ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#3D4756]'}`}
            ...
            <Button
              ...
              className={`flex-1 h-11 rounded-xl text-white shadow-lg transition-all duration-300 ${
                isFormValid() 
                  ? 'bg-[#63E6BE] hover:bg-[#5DD5BE] hover:shadow-xl hover:scale-105' 
                  : `${isDark ? 'bg-[#161b22] text-gray-600' : 'bg-gray-400'} cursor-not-allowed`
              }`}
              style={{ fontWeight: 600 }}
            >
                  ...
                  >
                    ...
                    <div className="space-y-3 mt-4">
                      <div>
                        ...
                        <div className="space-y-2">
                          {[
                            ...
                            <button
                              key={option.value}
                              onClick={() => setFilterEstado(option.value as any)}
                              className={`w-full text-left px-4 py-2 rounded-lg transition-all duration-200 ${
                                filterEstado === option.value
                                  ? 'bg-[#63E6BE] text-white shadow-md'
                                  : `${isDark ? 'bg-[#0d1117] text-gray-300 hover:bg-[#161b22]' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`
                              }`}
                      ...
                      <Button
                        ...
                        className={`w-full h-10 rounded-lg transition-all ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#3D4756]'}`}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gradient-to-r from-[#63E6BE] to-[#3D4756]">
                <tr>
                  <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>
                    Nombre del Rol
                  </th>
                  <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>
                    Descripción
                  </th>
                  <th className="text-center p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>
                    Usuarios
                  </th>
                  <th className="text-center p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>
                    Estado
                  </th>
                  <th className="text-right p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>
                    Acciones
                  </th>
                </tr>
              </thead>
              <tbody>
                <AnimatePresence>
                  {filteredRoles.length === 0 ? (
                    <tr>
                      <td colSpan={5} className={`text-center p-8 ${textSecondary}`}>
                    ...
                    filteredRoles.map((rol, index) => (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <p className={textPrimary} style={{ fontSize: '14px' }}>
                            {rol.descripcion}
                          </p>
                          <p className={`${textSecondary} mt-1 flex items-center gap-1`} style={{ fontSize: '11px' }}>
                            <Calendar className="w-3 h-3" />
                            Creado: {new Date(rol.fechaCreacion).toLocaleDateString('es-ES')}
                          </p>
                        </td>
                        <td className="p-4">
                              ...
                              <Tooltip.Trigger asChild>
                                <div className="inline-block">
                                  <Switch
                                    ...
                                    disabled={rol.usuariosActivos > 0 && rol.estado}
                                    className={`${
                                ...
                                <Tooltip.Portal>
                                  ...
                                  >
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div>
                                        <p style={{ fontWeight: 600 }}>No se puede desactivar</p>
                                        <p className="text-xs opacity-90">
                                          Hay {rol.usuariosActivos} usuarios activos con este rol
                                        </p>
                                      </div>
                                    </div>
        ...
        <Dialog open={modalOpen} onOpenChange={setModalOpen}>
          <DialogContent className={`${modalBg} sm:max-w-[700px] max-h-[90vh] overflow-y-auto rounded-2xl border-2 ${border}`}>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-3">
                ...
                <div>
                  <span className={`${textPrimary} block`} style={{ fontSize: '22px', fontWeight: 700 }}>
                    {selectedRol ? 'Editar Rol' : 'Crear Nuevo Rol'}
                  </span>
                  <span className={`${textSecondary} block`} style={{ fontSize: '13px', fontWeight: 500 }}>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-6">
              <TabsList className={`grid w-full grid-cols-2 ${isDark ? 'bg-[#0d1117]' : 'bg-gray-100'}`}>
                <TabsTrigger 
                  value="datos" 
              ...
              <TabsContent value="datos" className="space-y-4 mt-4">
                ...
                <div className="p-4 bg-[#63E6BE] bg-opacity-10 rounded-xl border border-[#63E6BE]">
                  <p className={textPrimary} style={{ fontSize: '13px', fontWeight: 600 }}>
                    💡 Complete los datos básicos y luego vaya a la pestaña "Permisos" para asignar los permisos del rol.
                  </p>
                </div>
              </TabsContent>
              ...
              <TabsContent value="permisos" className="space-y-4 mt-4">
                ...
                <div className={`p-4 ${isDark ? 'bg-[#0d1117]' : 'bg-gray-50'} rounded-xl`}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <p className={`${textSecondary} text-xs mb-1`}>Nombre del Rol</p>
                      <p className={`${textPrimary} font-semibold`}>{formData.nombre || 'Sin especificar'}</p>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <div 
                          ...
                          style={{ background: formData.color }}
                        />
                        <span className={`${textPrimary} font-semibold text-xs`}>{formData.color}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h4 className={`${textPrimary} flex items-center gap-2`} style={{ fontSize: '16px', fontWeight: 700 }}>
                  ...
                  {Object.entries(permisosDisponibles).map(([modulo, permisos]) => (
                    ...
                    >
                      <h5 className={`${textPrimary} mb-3 capitalize`} style={{ fontSize: '15px', fontWeight: 700 }}>
                        {modulo}
                      </h5>
                      <div className="space-y-2">
                        {permisos.map((permiso) => (
                          <div
                            key={permiso.id}
                            className={`flex items-start gap-3 p-3 rounded-lg ${isDark ? 'hover:bg-[#161b22]' : 'hover:bg-gray-50'} transition-all cursor-pointer`}
                ...
                <div className="p-4 bg-[#63E6BE] bg-opacity-10 rounded-xl border border-[#63E6BE]">
                  <p className={textPrimary} style={{ fontSize: '14px', fontWeight: 600 }}>
                    Total de permisos seleccionados: <span className="text-[#63E6BE] font-bold">{formData.permisos.length}</span>
                  </p>
                </div>
              </TabsContent>
            </Tabs>
            ...
            <div className={`flex gap-3 mt-6 pt-6 border-t ${border}`}>
              <Button
                ...
                className={`flex-1 h-12 rounded-xl transition-all duration-300 ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#3D4756]'}`}
                style={{ fontWeight: 600 }}
              >
                <X className="w-4 h-4 mr-2" />
                Cancelar
              ...
              <Button
                ...
                className={`flex-1 h-12 rounded-xl shadow-lg transition-all duration-300 ${
                  isFormValid() 
                    ...
                    : `${isDark ? 'bg-[#161b22] text-gray-600' : 'bg-gray-400 text-gray-600'} cursor-not-allowed`
                }`}
                style={{ fontWeight: 600 }}
              >
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent className={`${modalBg} rounded-2xl border-2 ${border}`}>
            <AlertDialogHeader>
              <AlertDialogTitle className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-red-100 flex items-center justify-center">
                  <AlertCircle className="w-6 h-6 text-red-600" />
                </div>
                <span className={textPrimary} style={{ fontSize: '20px', fontWeight: 700 }}>
                  ¿Eliminar rol?
                </span>
              </AlertDialogTitle>
              <AlertDialogDescription className={`${textSecondary} ml-16`} style={{ fontSize: '14px' }}>
                ¿Estás seguro de eliminar el rol "{selectedRol?.nombre}"? Esta acción no se puede deshacer.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter className="mt-6">
              <AlertDialogCancel className={`h-11 rounded-xl ${isDark ? 'bg-[#161b22] hover:bg-[#1f6feb1a] text-white' : 'bg-gray-200 hover:bg-gray-300 text-[#3D4756]'}`} style={{ fontWeight: 600 }}>
                Cancelar
              </AlertDialogCancel>
              <AlertDialogAction
