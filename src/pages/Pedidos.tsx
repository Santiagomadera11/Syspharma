...
const generarPedidosEjemplo = (): Pedido[] => {
  ...
  const estados: Array<"Pendiente" | "En Proceso" | "Entregado" | "Cancelado"> =
    [
      "Pendiente",
      "Pendiente",
      "En Proceso",
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const descuento = productosSeleccionados.reduce(
      (sum, p) =>
    ...
    pedidos.push({
      ...
      historial: [
        ...
        ...(estado !== "Pendiente"
          ? [{ estado, fecha: new Date(fecha.getTime() + 3600000) }]
          : []),
      ],
    });
  }
...
export default function Pedidos({ user }: PedidosProps) {
  ...
  useEffect(() => {
    ...
    const categoriasActivas = categoriasFromStorage.filter(
  ...
  useEffect(() => {
    ...
    if (pendientes > 0) {
      setTimeout(() => {
        toast.info(
          `Tienes ${pendientes} pedido${pendientes > 1 ? "s" : ""} pendiente${
            pendientes > 1 ? "s" : ""
          }`,
          {
            duration: 5000,
  ...
  const calcularIva = () => {
    const baseImponible = formData.productos.reduce(
  ...
  const abrirModalEdicion = (pedido: Pedido) => {
    ...
    setFormData({
      ...
      ivaPorcentaje:
        (pedido.iva / (pedido.subtotal - pedido.descuento)) * 100 || 16,
      notas: pedido.notas || "",
  ...
  const guardarPedido = async () => {
    ...
    if (selectedPedido) {
      ...
      setPedidos(
        pedidos.map((p) => (p.id === selectedPedido.id ? pedidoActualizado : p))
      );
  ...
  return (
    <div className={`min-h-screen ${bgPrimary} p-8`}>
      ...
      <div className="max-w-[1800px] mx-auto space-y-8">
        ...
        {/* Búsqueda rápida */}
        <div
          className={`${bgCard} rounded-3xl p-6 border-2 ${border} shadow-lg`}
        >
          <h3
        ...
        <div className="flex items-center gap-4">
          ...
          <Select value={filtroEstado} onValueChange={setFiltroEstado}>
            <SelectTrigger
              className={`w-64 h-14 rounded-2xl border-2 ${inputBorder} ${inputBg} ${inputText}`}
            >
                  ...
                  >
                    ...
                    <td className="p-6">
                      <div className="flex justify-center">
                        <span
                          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-white ${getEstadoColor(
                            pedido.estado
                          )}`}
        ...
        >
          ...
          >
            ...
            <div className={`${bgCard} rounded-2xl p-6 border-2 ${border}`}>
              ...
              {(busquedaProducto || categoriaFiltro !== "Todas") &&
        ...
        >
          <AlertDialogHeader>
            ...
            >
              ...
              {selectedPedido?.codigo} será eliminado permanentemente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              className={`${
                isDark
                  ? "bg-gray-700 hover:bg-gray-600"
                  : "bg-gray-200 hover:bg-gray-300"
              } ${textPrimary} rounded-xl h-11 px-6`}
            >
              Cancelar
            </AlertDialogCancel>
            <AlertDialogAction
