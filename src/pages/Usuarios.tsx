...
import { Plus, Search, Edit, Trash2, Eye, EyeOff, X, Check, Filter, FileText, Info, ArrowUpDown } from 'lucide-react';
...
import { validateFullName, validateEmail as validateEmailUtil, validatePhone, validateDNI, onlyNumbers, onlyLetters } from '../utils/validation';
...
export default function Usuarios({ user }: UsuariosProps) {
  const { isDark, bgCard, textPrimary, textSecondary, border, borderStrong, inputBg, inputBorder, tableHeader, tableRow, modalBg } = useDarkMode();
  ...
  const validateConfirmPassword = (confirmPassword: string, isEdit: boolean = false) => {
    ...
    if (formData.password && confirmPassword !== formData.password) return 'No coinciden';
    return '';
  };
  ...
  const handleBlur = (field: string) => {
    ...
    if (field === 'nombre') {
      ...
      setErrors({ ...errors, numeroDocumento: validateNumeroDocumento(formData.numeroDocumento) });
    } else if (field === 'password') {
      setErrors({ ...errors, password: validatePassword(formData.password, isEdit) });
    } else if (field === 'confirmPassword') {
      setErrors({ ...errors, confirmPassword: validateConfirmPassword(formData.confirmPassword, isEdit) });
    }
  };
  ...
  const handleChange = (field: string, value: string | boolean) => {
    ...
    if (touched[field as keyof typeof touched]) {
      ...
      if (field === 'nombre') {
        ...
        setErrors({ ...errors, tipoDocumento: validateTipoDocumento(value as string) });
      } else if (field === 'numeroDocumento') {
        setErrors({ ...errors, numeroDocumento: validateNumeroDocumento(value as string) });
      } else if (field === 'password') {
        setErrors({ ...errors, password: validatePassword(value as string, isEdit) });
        if (touched.confirmPassword) {
          setErrors(prev => ({ 
            ...prev, 
            confirmPassword: value !== formData.confirmPassword ? 'No coinciden' : '' 
          }));
        }
      } else if (field === 'confirmPassword') {
        setErrors({ ...errors, confirmPassword: validateConfirmPassword(value as string, isEdit) });
      }
    }
  };
  ...
  const isFormValid = () => {
    ...
    if (!formData.nombre || !formData.email || !formData.rol || !formData.tipoDocumento || !formData.numeroDocumento) return false;
    ...
    const confirmPasswordError = validateConfirmPassword(formData.confirmPassword, isEdit);
    
    return !nombreError && !emailError && !rolError && !tipoDocError && !numDocError && !passwordError && !confirmPasswordError;
  };
  ...
  const openCreateModal = () => {
    ...
    setErrors({ nombre: '', email: '', rol: '', tipoDocumento: '', numeroDocumento: '', password: '', confirmPassword: '' });
    setTouched({ nombre: false, email: false, rol: false, tipoDocumento: false, numeroDocumento: false, password: false, confirmPassword: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTouched({ nombre: true, email: true, rol: true, tipoDocumento: true, numeroDocumento: true, password: true, confirmPassword: true });
    ...
    if (nombreError || emailError || rolError || tipoDocError || numDocError || passwordError || confirmPasswordError) {
      return;
    }
    ...
    if (selectedUsuario) {
      ...
      toast.success('Usuario actualizado exitosamente', {
        style: { background: '#63E6BE', color: 'white', border: '1px solid #63E6BE' }
      });
    } else {
      const avatars = {
        'Administrador': 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=400&h=400&fit=crop',
        'Empleado': 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=400&h=400&fit=crop',
        'Cliente': 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=400&h=400&fit=crop'
      };
      ...
      const newUsuario: Usuario = {
        ...
        avatar: avatars[formData.rol as keyof typeof avatars] || `https://api.dicebear.com/7.x/avataaars/svg?seed=${formData.nombre}`,
  ...
  const filteredUsuarios = usuarios.filter(u => {
    const matchesSearch = u.nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         (u.numeroDocumento && u.numeroDocumento.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesRol = !filterRol || filterRol.trim() === '' || u.rol === filterRol;
    const matchesEstado = !filterEstado || filterEstado.trim() === '' || (filterEstado === 'activo' ? u.estado : !u.estado);
    const matchesTipoDoc = !filterTipoDoc || filterTipoDoc.trim() === '' || u.tipoDocumento === filterTipoDoc;
    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className={`${textSecondary} transition-colors duration-300 mt-1`} style={{ fontSize: '14px' }}>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <Input
              ...
              className={`pl-12 h-12 rounded-xl border-2 ${inputBorder} ${inputBg} ${isDark ? 'text-white placeholder-gray-400' : ''} focus:border-[#63E6BE] focus:ring-2 focus:ring-[#63E6BE]/20 transition-colors duration-300`}
            />
          </div>
          ...
          <Popover.Root open={showFilters} onOpenChange={setShowFilters}>
            ...
            <Popover.Portal>
              <Popover.Content
                className={`${isDark ? 'bg-[#1F2937]' : 'bg-white'} rounded-xl shadow-2xl ${isDark ? 'border-gray-700' : 'border-gray-200'} border p-4 w-[320px] z-50`}
                sideOffset={5}
              >
                <motion.div
                  ...
                  className="space-y-4"
                >
                  <h4 className={`${textPrimary}`} style={{ fontSize: '16px', fontWeight: 700 }}>
                    Filtros Avanzados
                  </h4>
          ...
          <table className="w-full">
            <thead className={tableHeader}>
              <tr>
                ...
                <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>Email</th>
                <th className="text-left p-4 text-white" style={{ fontWeight: 600, fontSize: '14px' }}>Rol</th>
                <th className="text-left p-4 text-white">
                  <button 
                    ...
                    >
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <span className={`${textPrimary} transition-colors duration-300`} style={{ fontWeight: 600, fontSize: '14px' }}>
                            {usuario.nombre}
                          </span>
                        </div>
                      </td>
                      <td className={`p-4 ${textSecondary} transition-colors duration-300`} style={{ fontSize: '14px' }}>
                        {usuario.email}
                      </td>
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          usuario.rol === 'Administrador' ? 'bg-purple-100 text-purple-700' :
                          usuario.rol === 'Empleado' ? 'bg-blue-100 text-blue-700' :
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  <SelectTrigger 
                    className={`h-11 rounded-xl ${inputBorder} ${inputBg} ${errors.tipoDocumento && touched.tipoDocumento ? 'border-red-500' : ''}`}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleChange('numeroDocumento', onlyNumbers(e.target.value))}
                  onBlur={() => handleBlur('numeroDocumento')}
                  className={`h-11 rounded-xl ${inputBorder} ${inputBg} ${isDark ? 'text-white' : ''} ${errors.numeroDocumento && touched.numeroDocumento ? 'border-red-500' : ''}`}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleChange('nombre', onlyLetters(e.target.value))}
                onBlur={() => handleBlur('nombre')}
                className={`h-11 rounded-xl ${inputBorder} ${inputBg} ${isDark ? 'text-white' : ''} ${errors.nombre && touched.nombre ? 'border-red-500' : ''}`}
            ...
            <div>
              ...
              <Input
                ...
                className={`h-11 rounded-xl ${inputBorder} ${inputBg} ${isDark ? 'text-white' : ''} ${errors.email && touched.email ? 'border-red-500' : ''}`}
            ...
            <div>
              ...
              >
                <SelectTrigger 
                  className={`h-11 rounded-xl ${inputBorder} ${inputBg} ${errors.rol && touched.rol ? 'border-red-500' : ''}`}
            ...
            <div>
              <label className={`block ${textPrimary} mb-2 font-semibold`} style={{ fontSize: '13px' }}>
                {selectedUsuario ? 'Cambiar Contraseña (opcional)' : 'Contraseña *'}
              </label>
              <div className="relative">
                <Input
                  ...
                  className={`h-11 rounded-xl pr-10 ${inputBorder} ${inputBg} ${isDark ? 'text-white' : ''} ${errors.password && touched.password ? 'border-red-500' : ''}`}
              ...
              {formData.password && (
                <div className="flex items-center gap-4 mt-2 text-xs">
                  <span className={`flex items-center gap-1 ${passwordValidation.minLength ? 'text-green-500' : 'text-gray-400'}`}>
                    {passwordValidation.minLength ? <Check className="w-3 h-3" /> : '○'}
                    Mínimo 8 caracteres
                  </span>
                  <span className={`flex items-center gap-1 ${passwordValidation.hasUpperCase ? 'text-green-500' : 'text-gray-400'}`}>
                    {passwordValidation.hasUpperCase ? <Check className="w-3 h-3" /> : '○'}
                    Una mayúscula
                  </span>
                  <span className={`flex items-center gap-1 ${passwordValidation.hasNumber ? 'text-green-500' : 'text-gray-400'}`}>
              ...
              <div>
                ...
                <div className="relative">
                  <Input
                    ...
                    className={`h-11 rounded-xl pr-10 ${inputBorder} ${inputBg} ${isDark ? 'text-white' : ''} ${errors.confirmPassword && touched.confirmPassword ? 'border-red-500' : ''}`}
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
            ...
            {/* Estado */}
            <div className="flex items-center justify-between p-4 rounded-xl" style={{ background: isDark ? 'rgba(99, 230, 190, 0.1)' : 'rgba(99, 230, 190, 0.1)' }}>
              <span className={`${textPrimary} font-semibold`} style={{ fontSize: '14px' }}>
                Usuario Activo
              </span>
              <Switch 
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button
                ...
                style={{ fontWeight: 600 }}
              >
                {loading ? 'Guardando...' : selectedUsuario ? 'Actualizar' : 'Crear Usuario'}
              </Button>
            </div>
          </form>
        </DialogContent>
          ...
          {selectedUsuario && (
            <div className="space-y-4 mt-4">
              <div className="flex justify-center">
                <img
                  src={selectedUsuario.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedUsuario.nombre}`}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className={`${textPrimary} font-semibold`}>{selectedUsuario.nombre}</p>
                </div>

                <div>
                  <p className={`${textSecondary} text-xs`}>Email</p>
                  <p className={`${textPrimary} font-semibold`}>{selectedUsuario.email}</p>
                </div>

                <div>
                  <p className={`${textSecondary} text-xs`}>Documento</p>
                  <p className={`${textPrimary} font-semibold`}>{selectedUsuario.tipoDocumento}: {selectedUsuario.numeroDocumento}</p>
                </div>

                <div>
                  <p className={`${textSecondary} text-xs`}>Rol</p>
                  <p className={`${textPrimary} font-semibold`}>{selectedUsuario.rol}</p>
                </div>

                {selectedUsuario.telefono && (
                  <div>
                    <p className={`${textSecondary} text-xs`}>Teléfono</p>
                    <p className={`${textPrimary} font-semibold`}>{selectedUsuario.telefono}</p>
                  </div>
                )}

                <div>
                  <p className={`${textSecondary} text-xs`}>Estado</p>
                  <p className={`${textPrimary} font-semibold`}>{selectedUsuario.estado ? 'Activo' : 'Inactivo'}</p>
                </div>
              </div>
        ...
        <AlertDialogContent className={`${modalBg} rounded-2xl`}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className={textSecondary}>
              Esta acción no se puede deshacer. Se eliminará permanentemente el usuario {selectedUsuario?.nombre}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
