...
export function initializeLocalStorage(): void {
  ...
  if (!localStorage.getItem(KEYS.MEDICOS)) {
    setItem(KEYS.MEDICOS, INITIAL_DATA.MEDICOS);
  }
}
...
export const proveedoresStorage = {
  getAll: (): Proveedor[] =>
    getItem(KEYS.PROVEEDORES, INITIAL_DATA.PROVEEDORES),
  save: (proveedores: Proveedor[]): void =>
    setItem(KEYS.PROVEEDORES, proveedores),
  ...
  update: (id: string, proveedorData: Partial<Proveedor>): Proveedor[] => {
    const proveedores = proveedoresStorage.getAll();
    const newProveedores = proveedores.map((p) =>
...
export const comprasStorage = {
  ...
  update: (id: string, compraData: Partial<Compra>): Compra[] => {
    const compras = comprasStorage.getAll();
    const newCompras = compras.map((c) =>
      c.id === id ? { ...c, ...compraData } : c
    );
    comprasStorage.save(newCompras);
    return newCompras;
  },
...
export const citasStorage = {
  ...
  update: (id: string, citaData: Partial<Cita>): Cita[] => {
    const citas = citasStorage.getAll();
    const newCitas = citas.map((c) =>
      c.id === id ? { ...c, ...citaData } : c
    );
    citasStorage.save(newCitas);
    return newCitas;
  },
...
// CRUD para Médicos (solo para agendamiento, no usuarios de sistema)
export const medicosStorage = {
  getAll: (): Medico[] => getItem(KEYS.MEDICOS, INITIAL_DATA.MEDICOS),
  save: (medicos: Medico[]): void => setItem(KEYS.MEDICOS, medicos),
  add: (medico: Medico): Medico[] => {
    const medicos = medicosStorage.getAll();
    const newMedicos = [...medicos, medico];
    medicosStorage.save(newMedicos);
    return newMedicos;
  },
  update: (id: string, medicoData: Partial<Medico>): Medico[] => {
    const medicos = medicosStorage.getAll();
    const newMedicos = medicos.map((m) =>
      m.id === id ? { ...m, ...medicoData } : m
    );
    medicosStorage.save(newMedicos);
    return newMedicos;
  },
  delete: (id: string): Medico[] => {
    const medicos = medicosStorage.getAll();
    const newMedicos = medicos.filter((m) => m.id !== id);
    medicosStorage.save(newMedicos);
    return newMedicos;
  },
};
