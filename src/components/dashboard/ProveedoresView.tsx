...
import { validateRequired, validateEmail, validatePhone, validateNIT } from '../../utils/validation';
...
export function ProveedoresView() {
  ...
  const loadProveedores = () => {
    setLoading(true);
    setTimeout(() => {
      const saved = JSON.parse(localStorage.getItem('syspharma_proveedores') || '[]');
  ...
  const errors = {
    ...
    nit: touched.nit && !validateRequired(formData.nit)
    ...
    email: touched.email && !validateRequired(formData.email)
      ...
      ? 'Email inválido'
      : '',
    telefono: touched.telefono && !validateRequired(formData.telefono)
      ? 'Este campo es obligatorio'
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const allProveedores = JSON.parse(localStorage.getItem('syspharma_proveedores') || '[]');
    ...
    if (editingProveedor) {
      ...
      localStorage.setItem('syspharma_proveedores', JSON.stringify(allProveedores));
  ...
  const handleDelete = (proveedor: Proveedor) => {
    if (window.confirm(`¿Estás seguro de eliminar a ${proveedor.nombre}?`)) {
      const all = JSON.parse(localStorage.getItem('syspharma_proveedores') || '[]');
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <InputWithValidation
            ...
            success={touched.telefono && !errors.telefono && formData.telefono.length > 0}
          ...
          <div className="flex gap-3 pt-4">
            ...
            <ButtonPastel variant="pink" type="button" onClick={() => setModalOpen(false)} className="flex-1">
              Cancelar
            </ButtonPastel>
          </div>
        </form>
