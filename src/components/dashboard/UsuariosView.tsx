...
import { Plus, Mail, User as UserIcon, Phone, Lock, Shield } from 'lucide-react';
import { Modal } from '../ui/modal';
import { Table } from '../ui/table';
import { InputWithValidation } from '../ui/input-with-validation';
import { ButtonPastel } from '../ui/button-pastel';
import { validateEmail, validateRequired, validatePhone, validatePassword } from '../../utils/validation';
...
export function UsuariosView() {
  ...
  const errors = {
    ...
    phone: touched.phone && formData.phone && !validatePhone(formData.phone)
      ...
      : '',
    password: !editingUser && touched.password && !passwordValidation.isValid
      ? 'Contraseña debe tener 8+ caracteres, mayúscula y número'
      : '',
    confirmPassword: !editingUser && touched.confirmPassword && formData.password !== formData.confirmPassword
      ? 'Las contraseñas no coinciden'
      : ''
  };

  const isValid = editingUser
    ? validateRequired(formData.name) && validateEmail(formData.email) && (!formData.phone || validatePhone(formData.phone))
    : validateRequired(formData.name) && validateEmail(formData.email) && (!formData.phone || validatePhone(formData.phone)) && passwordValidation.isValid && formData.password === formData.confirmPassword;
  ...
  const columns = [
    ...
    { key: 'phone', label: 'Teléfono', render: (user: Usuario) => user.phone || '-' },
    {
      key: 'role',
      label: 'Rol',
      render: (user: Usuario) => {
    ...
    },
    {
      ...
      render: (user: Usuario) => (
        <span
          className="px-3 py-1 rounded-lg text-sm"
          style={{ backgroundColor: user.active ? '#A7F3D0' : '#FBCFE8', color: '#374151' }}
        >
