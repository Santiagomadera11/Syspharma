...
export function ComprasView() {
  ...
  const loadCompras = () => {
    setLoading(true);
    setTimeout(() => {
      const saved = JSON.parse(localStorage.getItem('syspharma_compras') || '[]');
      setCompras(saved);
      setLoading(false);
    }, 500);
  };
  ...
  const columns = [
    { key: 'fecha', label: 'Fecha', render: (c: Compra) => new Date(c.fecha).toLocaleDateString() },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <ButtonPastel variant="green" onClick={() => toast.info('Módulo en desarrollo')}>
