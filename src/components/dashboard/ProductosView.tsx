...
import { validateRequired, validatePositiveNumber, validateNonNegativeNumber } from '../../utils/validation';
...
export function ProductosView() {
  ...
  const loadProductos = () => {
    setLoading(true);
    setTimeout(() => {
      const saved = JSON.parse(localStorage.getItem('syspharma_productos') || '[]');
      setProductos(saved);
      setLoading(false);
    }, 500);
  };
  ...
  const errors = {
    codigo: touched.codigo && !validateRequired(formData.codigo) ? 'Este campo es obligatorio' : '',
    nombre: touched.nombre && !validateRequired(formData.nombre) ? 'Este campo es obligatorio' : '',
    precio: touched.precio && !validateRequired(formData.precio)
      ? 'Este campo es obligatorio'
      : touched.precio && !validatePositiveNumber(Number(formData.precio))
      ? 'Debe ser mayor a 0'
      : '',
    stock: touched.stock && !validateRequired(formData.stock)
      ? 'Este campo es obligatorio'
      : touched.stock && !validateNonNegativeNumber(Number(formData.stock))
      ? 'Debe ser mayor o igual a 0'
      : '',
    categoria: touched.categoria && !validateRequired(formData.categoria) ? 'Este campo es obligatorio' : ''
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const allProducts = JSON.parse(localStorage.getItem('syspharma_productos') || '[]');
    ...
    if (editingProduct) {
      ...
      localStorage.setItem('syspharma_productos', JSON.stringify(updatedProducts));
  ...
  const handleDelete = (product: Producto) => {
    if (window.confirm(`¿Estás seguro de eliminar ${product.nombre}?`)) {
      const allProducts = JSON.parse(localStorage.getItem('syspharma_productos') || '[]');
      const updatedProducts = allProducts.filter((p: Producto) => p.id !== product.id);
      localStorage.setItem('syspharma_productos', JSON.stringify(updatedProducts));
    ...
    },
    {
      ...
      render: (product: Producto) => (
        <span
          ...
          style={{
            backgroundColor: product.stock > 10 ? '#A7F3D0' : product.stock > 0 ? '#FDE047' : '#FBCFE8',
            color: '#374151'
          }}
        >
          {product.stock}
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p style={{ color: '#9CA3AF' }}>Gestiona el inventario de productos</p>
        </div>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          <InputWithValidation
            ...
            error={errors.codigo}
            success={touched.codigo && !errors.codigo && formData.codigo.length > 0}
          ...
          <InputWithValidation
            ...
            error={errors.nombre}
            success={touched.nombre && !errors.nombre && formData.nombre.length > 0}
          ...
          <div className="grid grid-cols-2 gap-4">
            <InputWithValidation
              ...
              error={errors.precio}
              success={touched.precio && !errors.precio && formData.precio.length > 0}
              showValidation={touched.precio && formData.precio.length > 0}
            />
            ...
            <InputWithValidation
              ...
              error={errors.stock}
              success={touched.stock && !errors.stock && formData.stock.length > 0}
              showValidation={touched.stock && formData.stock.length > 0}
            />
          </div>
          ...
          <InputWithValidation
            ...
            success={touched.categoria && !errors.categoria && formData.categoria.length > 0}
            showValidation={touched.categoria && formData.categoria.length > 0}
          />

          <div className="flex gap-3 pt-4">
            <ButtonPastel variant="green" type="submit" disabled={!isValid} className="flex-1">
              {editingProduct ? 'Actualizar' : 'Crear Producto'}
            </ButtonPastel>
            <ButtonPastel variant="pink" type="button" onClick={handleCloseModal} className="flex-1">
              Cancelar
            </ButtonPastel>
          </div>
        </form>
